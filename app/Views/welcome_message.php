...
<!-- public function CardWithDate()
            { 

                $findClient=$this->ClientModel->findAll();
                $data=$this->request->getPost();

                if(!$this->ReservationsModel->areDatesAvailable($data['id_apartment'],
                    $data['start_reservations'], $data['end_reservations'])){
                    session()->set('error','dates chosed are no more available');
                    return redirect()->to('/CardWithDates');
                }

                
                
                if(!$this->ReservationsModel->save($data))
                {
                    session()->set('error','Error when reserving');

                    return redirect()->to('CardWithDates');
                    // $data['erreurs']=$this->ReservationsModel->errors();    
                }

                session()->set('profile',$data);
                session()->set('success','reservation done');
                
                // return redirect()->to('CreatingClient');
                return redirect()->to('updateClient');
            } -->

            <!-- Ca c'est pour les dates -->
